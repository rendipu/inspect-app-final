...
function QRModal({ unit, onClose }) {
  ...
  const handleDownload = () => {
    ...
    ctx.fillText(unit.nomor_unit, c.width / 2, 220 + pad + 26)
    ctx.fillStyle = '#78716c'
    ctx.font = '12px sans-serif'
    ctx.fillText(`${unit.brand} ${unit.tipe}`, c.width / 2, 220 + pad + 46)
  ...
  const handlePrint = () => {
    ...
    win.document.write(`
      ...
      <style>
        body{margin:0;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;font-family:sans-serif;padding:20px;box-sizing:border-box}
        img{width:220px;height:220px}
        h2{font-size:18px;margin:12px 0 4px;font-family:monospace}
        p{font-size:13px;color:#666;margin:0 0 8px}
        small{font-size:10px;color:#999;word-break:break-all;max-width:260px;text-align:center;display:block}
      ...
      <div style={{ background:'var(--sf)', borderRadius:16, padding:28, width:'100%', maxWidth:340, textAlign:'center', boxShadow:'0 20px 60px rgba(0,0,0,.2)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
          <div style={{ textAlign:'left' }}>
            ...
            <div style={{ fontSize:12, color:'var(--t3)' }}>Scan untuk lihat history publik</div>
          </div>
          <button onClick={onClose} style={{ background:'transparent', border:'none', fontSize:20, cursor:'pointer', color:'var(--t3)', lineHeight:1, padding:4 }}>✕</button>
        </div>
        ...
        <div style={{ background:'var(--bd2)', borderRadius:8, padding:'7px 12px', marginBottom:16, fontSize:11, color:'var(--t3)', wordBreak:'break-all', textAlign:'left' }}>
          🔗 {url}
        </div>
...
function UsersTab({ users, refetch }) {
  ...
  const saveEdit = async () => {
    setSaving(true)
    try { await api.updateUser(editRow.id, editRow); await refetch(); setEditRow(null) }
    catch (e) { alert(e.message) } finally { setSaving(false) }
  }
...
function UnitsTab({ units, refetch }) {
  ...
  const [saving,   setSaving]   = useState(false)
  const [qrUnit,   setQrUnit]   = useState(null)   // ← state QR modal
                  ...
                  {editRow?._id === u._id ? (
                    ...
                    <>
                      ...
                      <td style={{ whiteSpace:'nowrap' }}>
                        ...
                        <button
                          onClick={() => setQrUnit(u)}
                          style={{ marginRight:6, padding:'4px 10px', borderRadius:6, border:'1.5px solid var(--purbd)', background:'var(--purbg)', color:'var(--pur)', fontSize:11, fontWeight:700, cursor:'pointer' }}
        ...
        <div className="card" style={{ marginBottom:12, background:'var(--sfy)' }}>
          ...
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:8, alignItems:'flex-start' }}>
            <div style={{ flex:1, minWidth:180 }}>
              <div style={{ fontSize:11, color:'var(--t3)', marginBottom:4 }}>Tipe Unit (kosong=semua)</div>
              <TagInput tags={form.unit_tipe} onChange={v => setForm(f => ({ ...f, unit_tipe: v }))} placeholder="Ketik tipe, Enter..." />
            </div>
            <div style={{ flex:1, minWidth:160 }}>
              <div style={{ fontSize:11, color:'var(--t3)', marginBottom:4 }}>Brand (kosong=semua)</div>
              <TagInput tags={form.brand} onChange={v => setForm(f => ({ ...f, brand: v }))} placeholder="Ketik brand, Enter..." />
            </div>
            ...
            {editRow?.id === q.id ? (
              ...
              <>
                <div style={{ display:'flex', alignItems:'center', gap:6, flex:1, flexWrap:'wrap' }}>
                  ...
                  {toArr(q.unit_tipe).map((t, i) => <span key={'t'+i} style={{ fontSize:10, color:'var(--inf)', background:'var(--infbg)', border:'1px solid var(--infbd)', padding:'1px 6px', borderRadius:4 }}>{t}</span>)}
                  {toArr(q.brand).map((b, i) => <span key={'b'+i} style={{ fontSize:10, color:'var(--pd)', background:'var(--purbg)', border:'1px solid var(--purbd)', padding:'1px 6px', borderRadius:4 }}>{b}</span>)}
                </div>
