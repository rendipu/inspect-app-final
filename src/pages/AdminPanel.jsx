...
const IS = { background:'var(--sf)',border:'1.5px solid var(--bd)',color:'var(--t)',borderRadius:8,padding:'7px 10px',fontFamily:'inherit',fontSize:12,outline:'none' }
...
function UsersTab({ users, refetch }) {
  ...
  const saveNew = async () => {
    ...
    setSaving(true)
    try { await api.createUser(form); await refetch(); setForm({nrp:'',nama:'',jabatan:'',role:'mekanik',password:''}); setShowForm(false) }
    catch(e){ alert(e.message) } finally { setSaving(false) }
  }
  const saveEdit = async () => {
    setSaving(true)
    try { await api.updateUser(editRow._id, editRow); await refetch(); setEditRow(null) }
    catch(e){ alert(e.message) } finally { setSaving(false) }
  }
  const remove = async (_id) => {
    if (!confirm('Hapus user ini?')) return
    try { await api.deleteUser(_id); await refetch() } catch(e){ alert(e.message) }
  }
        ...
        <div className="card" style={{ marginBottom:12,background:'var(--sfy)' }}>
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8 }} className="g3">
            {[['NRP *','nrp','text'],['Nama *','nama','text'],['Jabatan','jabatan','text']].map(([p,k,t]) => (
              <input key={k} placeholder={p} type={t} value={form[k]} onChange={e=>setForm(v=>({...v,[k]:e.target.value}))} style={{...IS,width:'100%'}} />
            ))}
            ...
            <input placeholder="Password *" type="password" value={form.password} onChange={e=>setForm(v=>({...v,password:e.target.value}))} style={{...IS,width:'100%'}} />
            <button className="btn-ok" onClick={saveNew} disabled={saving}>{saving?'...':'Simpan'}</button>
          </div>
        </div>
      )}
      <div className="card" style={{ padding:0,overflow:'hidden' }}>
        <div className="ptbl"><table className="tbl">
          <thead><tr><th>NRP</th><th>Nama</th><th>Jabatan</th><th>Role</th><th>Aksi</th></tr></thead>
          <tbody>{users && users.map(u => (
            <tr key={u._id}>
              {editRow?._id===u._id ? (<>
                ...
                <td><select value={editRow.role} onChange={e=>setEditRow(v=>({...v,role:e.target.value}))} style={IS}><option value="mekanik">mekanik</option><option value="group_leader">group_leader</option><option value="admin">admin</option></select></td>
                <td style={{ whiteSpace:'nowrap' }}><button className="btn-ok btn-sm" style={{ marginRight:4 }} onClick={saveEdit} disabled={saving}>✓</button><button className="btn-g btn-sm" onClick={()=>setEditRow(null)}>✕</button></td>
              </>) : (<>
                ...
                <td style={{ whiteSpace:'nowrap' }}><button className="btn-g btn-sm" style={{ marginRight:6 }} onClick={()=>setEditRow({...u})}>Edit</button><button className="btn-err btn-sm" onClick={()=>remove(u.id)}>Hapus</button></td>
              </>)}
            </tr>
          ))}</tbody>
        </table></div>
...
function UnitsTab({ units, refetch }) {
  ...
  const saveNew = async () => {
    ...
    setSaving(true)
    try { await api.createUnit(form); await refetch(); setForm({nomor_unit:'',tipe:'',brand:'',model:'',tahun:'',hm:''}); setShowForm(false) }
    catch(e){ alert(e.message) } finally { setSaving(false) }
  }
  const saveEdit = async () => {
    setSaving(true)
    try { await api.updateUnit(editRow.id,editRow); await refetch(); setEditRow(null) }
    catch(e){ alert(e.message) } finally { setSaving(false) }
  }
  ...
  return (
    <div>
        ...
        <div className="card" style={{ marginBottom:12,background:'var(--sfy)' }}>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8 }} className="g3">
            {[['No. Unit *','nomor_unit'],['Tipe *','tipe'],['Brand *','brand'],['Model','model']].map(([p,k])=>(
              <input key={k} placeholder={p} value={form[k]} onChange={e=>setForm(v=>({...v,[k]:e.target.value}))} style={{...IS,width:'100%'}} />
            ))}
            <input placeholder="Tahun" type="number" value={form.tahun} onChange={e=>setForm(v=>({...v,tahun:e.target.value}))} style={{...IS,width:'100%'}} />
            <input placeholder="Hour Meter" type="number" value={form.hm} onChange={e=>setForm(v=>({...v,hm:e.target.value}))} style={{...IS,width:'100%'}} />
            <button className="btn-ok" onClick={saveNew} disabled={saving}>{saving?'...':'Simpan'}</button>
          </div>
        </div>
      )}
      <div className="card" style={{ padding:0,overflow:'hidden' }}>
        <div className="ptbl"><table className="tbl">
          <thead><tr><th>No. Unit</th><th>Tipe</th><th>Brand</th><th>Model</th><th>Tahun</th><th>HM</th><th>Aksi</th></tr></thead>
          <tbody>{units.map(u=>(
            <tr key={u.id}>
              {editRow?.id===u.id ? (<>
...
function QuestionsTab({ questions, refetch }) {
  ...
  const saveNew = async () => {
    ...
    setSaving(true)
    try { await api.createQuestion(form); await refetch(); setForm({kategori:'',pertanyaan:'',urutan:'',unit_tipe:'',brand:''}); setShowForm(false) }
    catch(e){ alert(e.message) } finally { setSaving(false) }
  }
  const saveEdit = async () => {
    setSaving(true)
    try { await api.updateQuestion(editRow.id,editRow); await refetch(); setEditRow(null) }
    catch(e){ alert(e.message) } finally { setSaving(false) }
  }
  const remove = async (id) => {
      ...
      {showForm && (
        <div className="card" style={{ marginBottom:12,background:'var(--sfy)' }}>
          <div style={{ display:'flex',gap:8,flexWrap:'wrap' }}>
            ...
            <input placeholder="Tipe Unit (kosong=semua)" value={form.unit_tipe} onChange={e=>setForm(v=>({...v,unit_tipe:e.target.value}))} style={{...IS,width:160}} />
            <input placeholder="Brand (kosong=semua)" value={form.brand} onChange={e=>setForm(v=>({...v,brand:e.target.value}))} style={{...IS,width:140}} />
            <button className="btn-ok" onClick={saveNew} disabled={saving}>{saving?'...':'Simpan'}</button>
          </div>
          <div style={{ marginTop:8,fontSize:11,color:'var(--t3)' }}>💡 Kosongkan Tipe Unit &amp; Brand agar pertanyaan berlaku untuk semua unit</div>
        </div>
      )}
      <div style={{ display:'flex',flexDirection:'column',gap:6 }}>
        {[...questions].sort((a,b)=>a.urutan-b.urutan).map(q=>(
          <div key={q.id} className="scard" style={{ display:'flex',alignItems:'center',justifyContent:'space-between',gap:10 }}>
            {editRow?.id===q.id ? (
              <div style={{ display:'flex',gap:8,flex:1,flexWrap:'wrap' }}>
                ...
                <input value={editRow.unit_tipe||''} placeholder="Tipe" onChange={e=>setEditRow(v=>({...v,unit_tipe:e.target.value||null}))} style={{...IS,width:100}} />
                <input value={editRow.brand||''} placeholder="Brand" onChange={e=>setEditRow(v=>({...v,brand:e.target.value||null}))} style={{...IS,width:90}} />
  ...
  return (
    <div>
      <div className="card" style={{ marginBottom:12,background:'var(--sfy)' }}>
        ...
        <div style={{ marginBottom:10 }}>
          ...
          <select value={form.unit_id} onChange={e=>setForm(v=>({...v,unit_id:e.target.value}))} style={{...IS,width:'100%'}}>
            ...
            {units.filter(u=>!recurring.find(r=>r.unit_id===u.id)).map(u=>(
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
        {recurring.map(s=>(
          <div key={s.id} className="scard" style={{ display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,flexWrap:'wrap', opacity: s.aktif ? 1 : 0.5 }}>
            <div>
      ...
      <div style={{ display:'flex',gap:8,marginBottom:20,flexWrap:'wrap',borderBottom:'1.5px solid var(--bd)',paddingBottom:12 }}>
        {TABS.map(t=>(
          <button key={t.k} onClick={()=>setTab(t.k)}
            style={{ padding:'6px 16px',borderRadius:20,fontSize:13,fontWeight:700,cursor:'pointer',border:'1.5px solid',background:tab===t.k?'var(--p)':'transparent',color:tab===t.k?'#1c1917':'var(--t3)',borderColor:tab===t.k?'var(--p)':'var(--bd)',transition:'all .15s' }}>
            {t.l}
          </button>
        ))}
      </div>
